...
| `controlVideo` / `control_video` | `sessionId`, tagged `command` | `void` | Rejects stale sessions. Playback commands are queued to the worker; execution failure arrives as `controlError`; property reads keep the snapshot authoritative. Fullscreen completes on the GTK thread before acknowledgement. Supports play, pause, non-negative seek, volume 0–1, mute, rate 0.25–4, nonblank track IDs, and native-window fullscreen set/toggle. Toggle computes the next state atomically on the GTK thread. |
