...
- `ensure_asset_thumbnail` handles one asset at high priority and waits for its scheduler result with a 180-second budget (`DEMAND_JOB_TIMEOUT`); exceeding it fails the command with a controlled timeout error instead of hanging. It returns `null` for an unknown asset, missing source, render failure, or a version lost to a concurrent re-index.
- `ensure_page_thumbnails` de-duplicates the supplied IDs (still without a positive-ID filter), clamps the batch to 256 unique IDs (`MAX_PAGE_BATCH`), and treats every found asset as high priority. Waiting uses a 180-second stall deadline: if no completed result arrives within that window the command fails instead of blocking forever. It returns `ready` items and `failed` IDs, and emits the legacy `thumbnail-ready` application event for every ready item.
