...
- `file_name` is the final non-empty segment of `path`, accepting both slash styles. `file_name_key` is `lower(file_name)`. Asset upsert, rename, and mapped bundle import update both fields together. Duplicate lookup and CSV import use the lowercase key; they do not compare full paths or file contents.
