...
impl ThumbnailScheduler {
    ...
    fn with_processor(worker_count: usize, ffmpeg_path: PathBuf, processor: TaskProcessor) -> Self {
...
fn worker_loop(shared: Arc<SchedulerShared>) {
    loop {
        ...
        let thumb_path = (shared.processor)(shared.ffmpeg_path.as_path(), &task);
...
mod tests {
    ...
    fn task(asset_id: i64, target: &str) -> ThumbnailTask {
        ThumbnailTask {
            asset_id,
            source_path: format!("C:/tmp/source-{asset_id}.jpg").into(),
