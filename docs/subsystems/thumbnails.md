...
One process-wide `ThumbnailScheduler` detects logical CPU capacity at startup with Rust's `available_parallelism()`. If detection fails, both modes use four workers. Browsing retains its total concurrency of `available_parallelism - 2`, clamped to 2 through 8, and at most two simultaneous videos. Bulk rendering uses one worker per available logical CPU for both total jobs and videos. The pool starts enough threads for the larger of the browsing and bulk limits; extra threads wait on the scheduler condition variable while browsing. The explicit worker-count constructor still enforces at least one worker for tests and direct callers. Startup refuses to launch the application when any worker thread fails to spawn.
...
Jobs are keyed by the target path string. Enqueuing an existing target job attaches a waiter rather than starting another render, up to 256 waiters per target; the next enqueue fails fast. Each waiter retains its own asset ID and receives the shared result. This de-duplicates work across entry points while bounding fan-out memory. Workers admit new jobs only below the active mode's total and video limits. Completion removes the job, decrements total-job and video accounting, wakes blocked workers, and sends the result to all waiters. A dropped receiver is ignored.

The processor uses `catch_unwind` so a job panic becomes a failed result and the worker loop continues. `MAX_PENDING_JOBS = 2048` bounds distinct queued and running jobs together; further enqueues fail with a queue-full error. Demand waits also have the deadlines described below. These controls do not make decoding or process shutdown infallible.
...
Only one `render_all_thumbnails` or `render_failed_thumbnails` operation may coordinate work at a time. Both automatically enter the shared scheduler's bulk mode, including for video thumbnails. There is no worker-count preference or dialog. A scoped guard restores browsing admission on success, cancellation, error, or panic; another guard clears the running and cancellation flags. Already-running jobs finish, and new jobs start only when the restored total and video limits permit them. Gallery requests retain high priority and share target deduplication with bulk jobs.

The coordinator keeps at most `min(2048, max(24, 3 × CPU count))` scheduler results pending. For example, 16 CPUs permit 16 simultaneous renders and a 48-result submission window; 32 CPUs permit 32 simultaneous renders and a 96-result window. The scheduler-wide 2,048-job bound still applies across all callers. CPU count means logical CPUs available to this process at startup; decoding and storage can limit actual throughput.

Both bulk modes fetch keyset pages sized to the larger of 512 assets and the submission window, capped at 2,048 rows by the database helper. Pages include failure eligibility and stop at the maximum asset ID observed at the start. They release read cursors/connections before waiting for render results. Render-all counts an existing current target as generated, records missing sources, and skips eligible failures. Retry-failed selects only failures whose `source_record_version` matches the asset, in asset-ID order. Success clears only that version's failure marker; an older worker cannot overwrite or clear a newer failure.

Each page drains its pending jobs and commits at most one page of collected path updates, bounded by 2,048 rows. The source compare-and-set and clear epoch reject obsolete updates; rejected IDs contribute to the separate stale count. The 180-second result-stall deadline remains in effect. Progress counts describe this evolving library; concurrent scans can alter eligibility after the initial count.
...
6. Preserve scheduler target de-duplication, waiter fan-out, low-to-high promotion, FIFO priority, image progress around blocked videos, mode-specific worker bounds, the browsing two-video cap, scoped bulk restoration, panic-safe terminal results, and the `MAX_PENDING_JOBS` queue bound. Startup must keep refusing to run when worker threads fail to spawn.
...
9. Treat cancellation as a partial commit. Test cancellation before enqueue, while below and at the CPU-based submission window, during final drain, with image/video jobs still running, and verify summary counts, failure rows, published files, and subsequent reconciliation.
...
- `src-tauri/src/services/thumb_scheduler.rs` covers CPU selection and fallback, bulk image/video capacity through 32 CPUs, restoration with active jobs, target deduplication, waiter fan-out, priority promotion at queue capacity, panic-safe terminal results, version echo, and the queue bound.
- `src-tauri/src/services/thumb_service.rs` covers both bulk actions with 48 pending results, browsing restoration after success/cancellation/error/panic, queue-full failure with active videos, stall timeouts, cancellation flag behavior, reuse of an existing thumbnail, file-removal counts, the scan-update-versus-completion race (stale render dropped and its file removed), clear-epoch publication drops, and stale stream results followed by a lower request ID after reload.
- `src-tauri/src/db/thumbnails.rs` verifies candidate pages above 512 rows, the 2,048-row cap, and keyset continuation.
- `src-tauri/tests/thumbnail_bulk.rs` renders temporary images, GIF, and MP4 media through both bulk actions, decodes every published JPEG, and checks SQLite paths and failure-marker cleanup.
