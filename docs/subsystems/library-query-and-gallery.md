...
The grid measures its content width and its origin inside the actual scroll container. Tiles remain square with a 10-pixel gap and four rows of overscan on either side. The full-width gallery sits below a sticky search/tools header; bulk mode adds a 360px inspector at widths of at least 1000px. The existing resize observers measure the narrower grid and retain the visible anchor. Changes to tile size or column count invalidate row measurements and keep the previous first visible asset in view. The scroll container, gallery, and grid have resize observation; listeners are removed on unmount.
