...
- New gallery thumbnails stay transparent until loaded, then fade over the existing tile background with a 300 ms ease-in-out opacity animation. This includes cached images and tiles mounted during scrolling. A source change resets readiness; an unchanged-source rerender preserves it. Only the image participates in this fade, with no stagger or delay to queries, tile placement, controls, or selection feedback. Reduced-motion preferences show loaded images immediately. `ThumbnailImage` defaults to no fade outside the gallery.
...
- The status footer reports thumbnail generation only when `hasMore` is true, and reports “End of results” with a locale-formatted, pluralized result count when thumbnail generation is idle and `hasMore` is false. It does not represent asset-page loading. Production therefore normally treats the known session total as the end boundary. The count uses `assetCount ?? assets.length`, so a filtered query or sparse cache shows the query total whenever it is available. The centered footer follows the grid in normal document flow, with roughly 136px of vertical space, flexible dividers, a decorative success checkmark, and wrapping text. It scrolls with the gallery without covering the last row; empty collections retain their empty state and have no footer.

The focused gallery and App tests cover totals, sparse caches, footer visibility, and locale changes. The desktop case in `e2e/specs/ui.redesign.e2e.js` selected by `--mochaOpts.grep 'footer'` checks scroll geometry, the final row, typography, narrow windows, and bulk mode in English and Polish under both themes. Run it with `MEDIATAGGER_UI_REDESIGN=1` and the [isolated desktop setup](../development/testing.md#animation-geometry). It saves synthetic screenshots and measurements under `artifacts/ui-redesign/`.
...
- `src/components/UI/__tests__/ThumbnailImage.test.tsx` covers fade readiness for delayed/cached loads, failures, source replacement, unchanged-source rerenders, and previews with fading disabled. The opt-in [desktop thumbnail fade checks](../development/testing.md#gallery-thumbnail-fade) sample opacity during filtering and scrolling in both themes and with reduced motion.
