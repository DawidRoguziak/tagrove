...
`library_metadata.revision` starts at `1` and is incremented with one SQL update. It records the global mutation revision, independent of database migration versions. Public query responses report this value from their read snapshot.

Query starts, cached starts, page reads, and position validation use one deferred SQLite read transaction for global revision, dependency markers, and materialized rows.

The reserved `query_revision:` namespace stores global revision values for `general`, `favorites`, `tag_count`, and `tag:<normalized-name>`. A session depends on general, favorites when filtered, every included/excluded tag, and tag count under an exact-count filter. Its dependency revision is the maximum of these markers. Missing tag markers contribute zero. Exact parameterized lookups use batches of 500 keys, so punctuation never becomes a SQL wildcard.

General revision helpers atomically bump global revision and the general marker. Groups, scans, deletion, imports, restore, and clear retain full invalidation. Favorite mutations bump the favorites marker only when an existing value changes; single favorite no-ops and missing IDs still bump the global revision. Tag mutations report actual normalized set differences and whether any affected asset changed count. Bulk results union changed names and OR per-asset count changes. Any canonicalization or invariant repair uses full invalidation, including shared-tag repairs. Metadata, global revision, and affected markers commit together.

At the end of isolated initialization, one transaction replaces only the dependency namespace, setting the three fixed markers to the final global revision. Per-tag markers are created lazily and survive ordinary orphan cleanup. Startup and restore cannot retain sessions across this reset. No table, schema-version, or archive-format change is needed.

The ordered-ID query checks cooperative cancellation while building. Query-visible writes must commit their revision bump with the mutation. See [query session mechanics](library-query-and-gallery.md#backend-session-and-page-contract) for cache bounds, supersession, and invalidation; the [IPC contract](../architecture/ipc-contract.md#query-session-states) defines returned states.
...
Bulk thumbnail candidate reads use keyset pages with an explicit limit capped at 2,048 rows. Their coordinator sizes each page to accommodate the CPU-based submission window and publishes path updates once that page drains. See [bulk thumbnail scheduling](thumbnails.md#bulk-render-retry-and-cancellation).
...
- A session snapshots IDs and order, not complete asset rows. If query-visible code changes a row without a successful revision bump, later pages can materialize changed summaries against the old ID order. Media-group single-item setters deliberately invalidate for no-ops and missing IDs; favorite setters bump globally but preserve dependency markers in those cases; tag replacement is no-op-aware and rejects missing IDs.
- The query-start snapshot covers revision, ordered IDs, and the first page, but the read transaction is deferred: a write that commits between registration and the first read can still be included, which is safe. A relevant mutation committing after the snapshot produces a later `stale` transition. Unrelated metadata edits preserve the session and appear in subsequent summary reads.
